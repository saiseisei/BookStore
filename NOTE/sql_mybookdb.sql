create table if not exists bookstoredb.bookinfo(
	...
	COMMENT varchar(300),
	DELFLAG int(1)
)ENGINE = InnoDB, CHARSET = utf8, COMMENT = '書類情報';   
...
create table if not exists bookstoredb.userinfo(
	...
	AGE int(2),
	DELFLAG int(1)
)ENGINE=InnoDB, CHARSET = utf8, COMMENT = 'ユーザ情報';
...
create table if not exists bookstoredb.orderinfo(
	...
	QUANTITY integer,
	BUYDATE date
)ENGINE=InnoDB, CHARSET = utf8, COMMENT = '書類購買履歴';
...
create table if not exists bookstoredb.category(
	...
	COMMENT varchar(300),
	DELFLAG int(1)
)ENGINE = InnoDB, CHARSET = utf8, COMMENT = '書類カテゴリ情報';   
